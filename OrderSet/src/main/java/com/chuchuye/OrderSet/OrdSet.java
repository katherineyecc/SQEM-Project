...
public class OrdSet {
	...
	public boolean remove (int val) throws ArithmeticException{
		if (_overflow) throw new ArithmeticException();
	...
	/**
	 * Makes a free slot in the array, to place the value n into. 在数组中腾出一个空槽，用来将n放入
...
being made.
	 ...
	 * set _overflow
	 */
	public void set_overflow(boolean flag) {
		this._overflow = flag;
	}
...
/*	public boolean isEmptyStub() {
	 ...
	 * Stub of elementAt()
	 */
/*	public int elementAtStub(int n) {
		return 2;
	}
	 ...
	 * equals() using stubs
	 */
/*	public int equalsUsingStubs(OrdSet x) {
		int k,n;
		
		if (x.getSetLastStub() != _last) {
			return (_last - x.getSetLastStub());
		}
		
		if (isEmptyStub() && x.isEmptyStub()) return 0;
		
		for(k = 0, n = 0; n < (x.getSetLastStub() + 1); n++) {
			k = x.elementAtStub(n);
			if (k != _set[n]) {
				return 1;
			}
